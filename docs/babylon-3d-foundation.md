...
화면에는 20m × 20m 바닥, 코드로 만든 초록 큐브, GLB에서 읽은 갈색 큐브, 탑다운 카메라, 조명과 그림자, FPS 디버그 정보가 보여야 한다. 마우스 드래그로 회전하고 휠로 확대·축소한다. `R`을 누르면 카메라가 초기 위치로 돌아간다.
...
2026-08-24 로컬 Chrome 검증에서는 WebGL2로 장면과 GLB가 정상 표시됐고, 1920×1080 화면에서 약 165 FPS, 메시 4개를 확인했다. FPS는 컴퓨터와 브라우저 상태에 따라 달라진다.
