...
- `CROUCH`: 허리만 숙이지 않고 골반을 낮춘 뒤 허벅지와 정강이를 접어 앉은 자세를 만듦. 탐지 기준 높이도 함께 낮아짐
- `CROUCH_WALK`: 앉은 높이를 유지하고 팔을 몸 앞으로 모은 채 짧은 보폭으로 이동
...
- Chase 중에는 샘플 재생 속도를 약 1.32배로 올려 달려오는 발걸음으로 들린다.
- 경비 발소리는 플레이어와의 거리에 따라 크기와 좌우 방향이 변한다. 약 2.4m 안에서는 거의 최대 음량이고, 멀어질수록 빠르게 작아져 최대 13m까지만 들린다.
