...
# 2026-08-26 — 3D 13단계 결과·재시작·게임 오버

- 메인 메뉴, 일시정지, 재시작 확인, 성공·실패 결과 화면을 추가했다.
- 실행 시간, 발각, 추격, 선택 보물을 중복 없이 기록하고 점수·랭크·Perfect를 계산한다.
- 최고 점수와 최단 시간을 안전하게 로컬 저장하며 디버그 강제 결과는 기록에서 제외했다.
- 페이지를 새로고침하지 않는 중앙 `RunResetManager`로 월드·입력·UI·기록을 함께 초기화한다.
- 자세한 내용은 `docs/3d/result-restart-game-over-3d.md`에 정리했다.

# 2026-08-26 — 3D 14단계 웹 최적화

- 메인 메뉴 뒤 박물관 렌더링을 없애고 PLAY 이후 씬·오디오를 준비하는 지연 로딩으로 변경했다.
- 사용하지 않던 test GLB 런타임을 제거해 3D 번들 크기를 줄였다.
- LOW/MEDIUM/HIGH 품질, 적응형 Render Scale, Fullscreen과 Focus Lost 자동 일시정지를 추가했다.
- 경보 비콘과 탈출 마커 Instance, Guard 공용 재질, 원거리 애니메이션 저주기 업데이트를 적용했다.
- Guard LOS 15Hz, CCTV 10Hz와 임시 Vector 제거로 AI 판정 비용을 줄였다.
- 1080p 시작 화면의 제목·설명·조작 안내 크기와 대비를 높였다.
- 자세한 내용은 `docs/3d/web-optimization-3d.md`에 정리했다.
